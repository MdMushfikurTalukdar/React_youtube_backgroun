...
const VideoConverter = () => {
  ...
  const takeScreenshots = async () => {
    ...
    if (videoElement) {
      for (let i = 0; i < screenshotCount; i++) {
        // Set the current time of the video to the desired time for the screenshot
  ...
  const convertToBlackAndWhite = () => {
    const newProcessedScreenshots = screenshots.map((screenshot) => {
  ...
  const invertColors = () => {
    const newProcessedScreenshots = processedScreenshots.map((screenshot) => {
      const canvas = document.createElement('canvas');
      const ctx = canvas.getContext('2d');
      const img = new Image();
      img.src = screenshot;
